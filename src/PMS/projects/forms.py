...
class ProjectForm(forms.ModelForm):
    ...
    name = forms.CharField(required=True, label=_('name'), help_text=_('help_name'))
    short_name = forms.CharField(required=True, max_length=3, label=_('short_name'), help_text=_('help_short'),
                                 validators=[alphanumeric])
    desc = forms.CharField(required=True, label=_('desc'), help_text=_('help_desc'), widget=CKEditorUploadingWidget())
...
class ProjectSettingForm(forms.ModelForm):
    ...
    def __init__(self, *args, submit_title='Submit', **kwargs):
        ...
        self.helper = FormHelper()
        self.helper.form_tag = False
        self.fields["project"].widget = CheckboxSelectMultiple(attrs={"checked": ""})  # 下拉選項改成核取方塊
