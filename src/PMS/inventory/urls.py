...
from inventory.views import main, apply, TypeAPI, ItemAPI, detail, agree, apply_list, approve, reject, import_excel, \
    statistic, change_status, delete, mail_test, mail_reject, mail_agree, recieved, pr_apply, search
...
urlpatterns = [
    ...
    url(r'^agree/(?P<pk>\w+)/$', agree, name='inv_agree'),
    url(r'^reject/(?P<pk>\w+)/$', reject, name='inv_reject'),
    ...
    url(r'^mail_reject/(?P<pk>\w+)/$', mail_reject, name='inv_mail_reject'),
    url(r'^mail_agree/(?P<pk>\w+)/$', mail_agree, name='inv_mail_agree'),
